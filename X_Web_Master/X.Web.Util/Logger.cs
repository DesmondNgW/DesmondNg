...
namespace X.Web.Util
{
    ...
    public sealed class Logger
    {
        ...
        /// GetMethodParamList
        /// </summary>
        private static Dictionary<string, object> GetParamList(MethodBase declaringType, object[] values)
        {
            var arguments = declaringType.GetParameters().OrderBy(p => p.Position).ToList();
            if (arguments.Count.Equals(0) || arguments.Count != values.Length) return null;
        ...
        {
            NAction.BeginInvoke(IpBase.GetIp(), methodInfo, domain, LogType.Debug, returnValue, messages, null, null);
        }

        public static void Error(CoreMethodInfo methodInfo, LogDomain domain, object returnValue, params string[] messages)
        {
            NAction.BeginInvoke(IpBase.GetIp(), methodInfo, domain, LogType.Error, returnValue, messages, null, null);
        }
        ...
        {
            MAction.BeginInvoke(IpBase.GetIp(), declaringType, domain, LogType.Debug, returnValue, address, messages, null, null);
        }
        ...
        {
            MAction.BeginInvoke(IpBase.GetIp(), declaringType, domain, LogType.Info, returnValue, address, messages, null, null);
        }
    }
}
